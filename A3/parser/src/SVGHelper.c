...
// for creating a new SVG on the server
bool createNewSVG(char* fileName, char* schemaFile, char* desc, char* title){
    ...
    // border->width = 4.98;
    // border->height = 3.98;
    // strcpy(border->units,"cm");

    // Attribute *stroke = (Attribute*)malloc(sizeof(Attribute)*(sizeof(char)*strlen("0.2cm")+1));
    // stroke->name = malloc(sizeof(char)*(strlen("stroke-width")+1));
    // strcpy(stroke->name,"stroke-width");
    // strcpy(stroke->value,"0.2cm");
    
    // insertBack(border->otherAttributes, stroke);

    //insertBack(svg->rectangles,(void*)border);

    bool result = validateSVG(svg,schemaFile);
   // printf("filename: %s\n",fileName);
    ...
    //check for .svg extention
    char *ext = strrchr(fileName,'.');
    //printf("extension %s\n",ext);

    if(!ext){
        char* addSVG = malloc(sizeof(char)*(strlen(fileName)+5));
        strcpy(addSVG, fileName);
        strcat(addSVG,".svg");

        fileName = addSVG;
        //printf("filename with svg: %s\n",fileName);
    }
    
    //result = writeSVG(svg, fileName);
...
char* getTitle(char* fileName, char* schemaFile){
    ...
    char* title = malloc(sizeof(char)*(strlen(svg->title)+1));
...
char* getDesc(char* fileName, char* schemaFile){
    ...
    char* description = malloc(sizeof(char)*(strlen(svg->description)+1));
...
char* rectangleToJSONFromFile(char* fileName, char* schemaFile){
    ...
    char* rectJSON = rectListToJSON(getRects(svg));
...
char* circleToJSONFromFile(char* fileName, char* schemaFile){
    ...
    char* circleJSON = circListToJSON(getCircles(svg));
...
char* attributesToJSONFromFile(char* fileName, char* schemaFile, char* shape, int index){
    ...
    if(strcmp(shape,"RECT")==0){
        ListIterator iter = createIterator(getRects(svg));
        void* elem;
        int i = 0;
        while((elem = nextElement(&iter)) != NULL){
            Rectangle* rect = (Rectangle*)elem;
            if(index == i){
                char* attrListJSON = attrListToJSON(rect->otherAttributes);
                deleteSVG(svg);
                return attrListJSON;
            }
            i++;
    ...
    if(strcmp(shape,"CIRC")==0){
        ListIterator iter = createIterator(getCircles(svg));
        void* elem;
        int i = 0;
        while((elem = nextElement(&iter)) != NULL){
            Circle* circle = (Circle*)elem;
            if(index == i){
                char* attrListJSON = attrListToJSON(circle->otherAttributes);
                deleteSVG(svg);
                return attrListJSON;
            }
            i++;
    ...
    if(strcmp(shape,"PATH")==0){
        ListIterator iter = createIterator(getPaths(svg));
        void* elem;
        int i = 0;
        while((elem = nextElement(&iter)) != NULL){
            Path* path = (Path*)elem;
            if(index == i){
                char* attrListJSON = attrListToJSON(path->otherAttributes);
                deleteSVG(svg);
                return attrListJSON;
            }
            i++;
    ...
    if(strcmp(shape,"GROUP")==0){
        ListIterator iter = createIterator(getGroups(svg));
        void* elem;
        int i = 0;
        while((elem = nextElement(&iter)) != NULL){
            Group* group = (Group*)elem;
            if(index == i){
                char* attrListJSON = attrListToJSON(group->otherAttributes);
                deleteSVG(svg);
                return attrListJSON;
            }
            i++;
...
char* pathToJSONFromFile(char* fileName, char* schemaFile){
    ...
    char* pathJSON = pathListToJSON(getPaths(svg));
...
char* groupToJSONFromFile(char* fileName, char* schemaFile){
    ...
    char* groupJSON = groupListToJSON(getGroups(svg));
...
int validateFile(char* fileName, char* schemaFile){
    SVG* svg = createValidSVG(fileName,schemaFile);
    bool valid = validateSVG(svg,schemaFile);
    deleteSVG(svg);
    if(valid){
        return 1;
    }
...
// adding a rectangle from addShape
...
int addRectangleToSVGFromServer(char* fileName, char* schemaFile,float x, float y, float width, float height, char* units, char* fillAttr){
    ...
    //only allow to add a fill attribute

    if(strlen(fillAttr)>0){
        Attribute* attr = malloc(sizeof(Attribute)+strlen(fillAttr)+1);
        attr->name = malloc(strlen("fill")+1);

        strcpy(attr->name,"fill");
        strcpy(attr->value,fillAttr);
...
int addCircleToSVGFromServer(char* fileName, char* schemaFile,float cx, float cy, float r, char* units, char* fillAttr){
    ...
    if(strlen(fillAttr)>0){
        ...
        insertBack(circ->otherAttributes, attr);
    }
...
int scaleShapeFromServer(char* fileName, char* schemaFile, char* elemType, float scaleFactor){
    ...
    if(strcmp(elemType,"Rectangle")==0){

        ListIterator iter = createIterator(getRects(svg));
        void* elem;
        while((elem = nextElement(&iter)) != NULL){
            Rectangle* rect = (Rectangle*)elem;
            //printf("width before: %f\n",rect->width);
            rect->width *= scaleFactor;
            //printf("width after: %f\n",rect->width);
            rect->height *= scaleFactor;
        }

    }else if(strcmp(elemType,"Circle")==0){

        ListIterator iter = createIterator(getCircles(svg));
        void* elem;
        while((elem = nextElement(&iter)) != NULL){
            Circle* circle = (Circle*)elem;
            //printf("radius before: %f\n",circle->r);
...
//change or add attribute from file
...
bool changeOrAddAttribute(char* fileName, char* schemaFile,char* shapeType, int shapeNum, char* attributeName, char* attributeValue){
    ...
    Attribute* newAttribute = (Attribute*)malloc(sizeof(Attribute)+(sizeof(char)*(strlen(attributeValue)+1)));
    
    strcpy(newAttribute->value,attributeValue);

    newAttribute->name = malloc(sizeof(char)*(strlen(attributeName)+1));
    strcpy(newAttribute->name,attributeName);

    bool set;
    
    if(strcmp("Rectangle",shapeType)==0){
        set = setAttribute(svg,RECT,index,newAttribute);
    }
    if(strcmp("Circle",shapeType)==0){
        set = setAttribute(svg,CIRC,index,newAttribute);
    }
    if(strcmp("Path",shapeType)==0){
        //printf("setting attribute\n");
        set = setAttribute(svg,PATH,index,newAttribute);
        // if(setAttribute(svg,PATH,shapeNum,newAttribute)){
        //     printf("true\n");
        // }
        
    }
    if(strcmp("Group",shapeType)==0){
        set = setAttribute(svg,GROUP,index,newAttribute);
    }

    

    if(validateSVG(svg,schemaFile) && set){
        //printf("true\n");
        writeSVG(svg,fileName);
        deleteSVG(svg);
        return true;
